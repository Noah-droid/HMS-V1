    ...
    try:
        if request.method == 'POST':
            ...
            if capacity > occupied:
                ...
                message = "The room is already filled, Please select another room."
                messages.error(request, message)
                print(message)
                # url = 'rooms/floor_id/'
                return redirect('rooms', pk=floor_id)
    except:
        message = "You've already booked a room."
        messages.error(request, message)
        return redirect('home')

    context = {'roomlist': room_list, 'capacity':capacity, 'occupied':occupied}
...
def booking_form_views(request, pk):
    ...
    context = {'room_number': room_number, 'floor_number': floor_number, 'block_name': block_name, 'capacity':capacity, 'occupied':occupied}
